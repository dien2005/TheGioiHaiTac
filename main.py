...
class Game:
	def __init__(self):
		...
		self.current_stage = None #Level(self.tmx_maps[self.data.current_level], self.level_frames, self.audio_files, self.data, self.switch_stage)
		self.algorithm = 'DFS'
	...
	def switch_stage(self, target, unlock = 0):
		if target == 'level':
			self.current_stage = Level(self.tmx_maps[self.data.current_level], self.level_frames, self.audio_files, self.data, self.switch_stage, self.algorithm)
			...
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					...
					if event.key == pygame.K_ESCAPE:
						# Reset current_stage để menu không tự thoát
						self.current_stage = None
						# Quay lại menu để chọn thuật toán
						self.menu = Menu(self)  # Tạo mới để reset trạng thái chọn
						self.menu.run()
						continue  # Quay lại vòng lặp, đợi current_stage mới từ menu
