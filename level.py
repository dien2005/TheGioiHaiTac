...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage, algorithm='DFS'):
        ...
        # Tạo grid trước setup để đảm bảo có sẵn cho Tooth
        self.create_grid(tmx_map)
        if not hasattr(self, 'grid'):
            raise RuntimeError("Không thể tạo self.grid trong create_grid")
    ...
    def create_grid(self, tmx_map):
        self.grid = []
        for row in range(tmx_map.height):
            grid_row = []
            for col in range(tmx_map.width):
                # Ban đầu đánh dấu tất cả ô là đi được; sẽ cập nhật sau setup
                grid_row.append(1)  # 1: đi được, 0: bị chặn
            self.grid.append(grid_row)

    def update_grid(self):
        # Cập nhật grid dựa trên collision_sprites sau setup
        for row in range(len(self.grid)):
            for col in range(len(self.grid[0])):
                is_collision = any(
                    sprite.rect.collidepoint((col * TILE_SIZE + TILE_SIZE / 2, row * TILE_SIZE + TILE_SIZE / 2))
                    for sprite in self.collision_sprites
                )
                self.grid[row][col] = 0 if is_collision else 1
    ...
    def setup(self, tmx_map, level_frames, audio_files):
        ...
        for obj in tmx_map.get_layer_by_name('Enemies'):
            if obj.name == 'tooth':
                Tooth((obj.x, obj.y), level_frames['tooth'], (self.all_sprites, self.damage_sprites, self.tooth_sprites), self.collision_sprites, self.player, self, self.algorithm)
            if obj.name == 'shell':
                Shell(
                    pos=(obj.x, obj.y), 
        ...
        # Cập nhật grid sau setup để tính đến collision_sprites
        self.update_grid()
    ...
    def run(self, dt):
        ...
        # Cập nhật enemy Tooth
        self.tooth_sprites.update(dt)

        # Cập nhật toàn bộ sprite khác
